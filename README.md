...
**Застряла старая версия (после моего обновления)?** Один раз пробей кэш: на телефоне — закрой вкладку/приложение полностью и открой заново (обычно хватает); на компьютере — Ctrl+Shift+R (жёсткая перезагрузка). Если совсем упёрлось — в DevTools → Application → Service Workers → Unregister, потом обнови. После этого старый кэш стирается сам и больше мешать не будет.
